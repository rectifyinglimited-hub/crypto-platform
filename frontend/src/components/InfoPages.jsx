...
const LIME_BTN =
  "inline-flex items-center justify-center gap-2 rounded-md bg-[#C8FF00] px-7 py-3 text-sm font-extrabold uppercase tracking-wide text-[#1a1400] shadow-[0_0_28px_rgba(200, 255, 0,0.45)] transition hover:bg-[#E8FF8A]";

export function AboutPage({ onCta, onSupport, ctaLabel = "Open an account" }) {
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-[#C8FF00]/20 px-5 py-12 sm:px-10">
        <VideoBackdrop
          src={CRYPTO_VIDEO}
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {[
          ...
          <div key={c.title} className="rounded-2xl border border-[#C8FF00]/15 bg-black/40 p-5">
            <h3 className="font-bold text-[#C8FF00]">{c.title}</h3>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        ...
        {onSupport && (
          <button
            type="button"
            onClick={onSupport}
            className="inline-flex items-center justify-center gap-2 rounded-md border border-[#C8FF00]/40 px-6 py-3 text-sm font-extrabold uppercase tracking-wide text-[#C8FF00] hover:bg-[#C8FF00]/10"
          >
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-[#C8FF00]/20 px-5 py-12 sm:px-10">
        ...
        <div className="relative z-10 mx-auto max-w-xl text-center">
          <Headphones className="mx-auto h-10 w-10 text-[#C8FF00]" />
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-black/40 p-5">
          ...
          <div className="mt-3 flex items-start gap-2 text-sm leading-relaxed text-white/80">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#C8FF00]" />
            <p>
          ...
          <a
            ...
            className="mt-4 flex items-center gap-2 text-sm font-semibold text-[#C8FF00] hover:underline"
          >
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {SOCIAL_LINKS.map((s) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-xl border border-[#C8FF00]/25 px-4 py-2 text-sm font-semibold text-[#C8FF00] hover:bg-[#C8FF00]/10"
              >
                {s.label}
              </a>
            ))}
...
export function VipPage({ user, onCta, onSupport, onReferral }) {
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-[#C8FF00]/35 px-5 py-12 shadow-[0_0_50px_rgba(200, 255, 0,0.12)] sm:px-10">
        <VideoBackdrop
          src={CRYPTO_VIDEO}
          poster={CRYPTO_POSTER}
        ...
        <div className="relative z-10 grid items-center gap-8 lg:grid-cols-2">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#C8FF00]/40 bg-[#C8FF00]/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.2em] text-[#C8FF00]">
            ...
            {signedIn ? (
              <div className="mt-5 grid max-w-md grid-cols-2 gap-2 sm:grid-cols-3">
                <div className="rounded-xl border border-white/10 bg-black/45 px-3 py-2">
                  ...
                  <div className="mt-1 text-lg font-extrabold text-[#C8FF00]">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-2xl border border-[#C8FF00]/20 bg-black/45 p-5">
          <h3 className="flex items-center gap-2 font-bold text-[#C8FF00]">
            ...
            return (
              <div
                ...
                className={`rounded-2xl border p-3 ${
                  active
                    ? "border-[#C8FF00]/45 bg-[#C8FF00]/12"
                    : "border-white/10 bg-black/35"
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs font-extrabold text-[#C8FF00]">
                    {t.name || `VIP ${t.level}`}
                  </div>
                  {active ? (
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {[
          ...
          <div
            key={title}
            className="rounded-2xl border border-[#C8FF00]/20 bg-black/45 p-6 text-center"
          >
