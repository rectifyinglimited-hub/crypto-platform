...
function OfficialStamp({ ring, center, sub, id }) {
  ...
  return (
    <svg viewBox="0 0 140 140" className="h-[108px] w-[108px] sm:h-[124px] sm:w-[124px]">
      ...
      <circle cx="70" cy="70" r="61" fill="none" stroke="#C8FF00" strokeWidth="3.2" />
    ...
    <div className="mx-auto max-w-3xl pb-8">
      <div className="px-4 py-10 text-center sm:py-14">
        <div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-full border border-white/20">
          <svg viewBox="0 0 24 24" className="h-6 w-6 text-[#C8FF00]" fill="none" stroke="currentColor" strokeWidth="1.6">
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-2 rounded-full bg-[#C8FF00] px-6 py-2.5 text-sm font-extrabold text-black"
        >
...
export function CertificatePreview({ onOpen }) {
  return (
    <button
      type="button"
      onClick={onOpen}
      className="w-full overflow-hidden rounded-2xl border border-[#C8FF00]/25 text-left shadow-[0_12px_40px_rgba(0,0,0,0.35)] transition hover:border-[#C8FF00]"
      title="Click to open official certificate"
    >
      <div className="origin-top scale-[0.98]">
        <OfficialCertificateDocument />
      </div>
      <div className="bg-[#0b0e11] px-4 py-3 text-center text-[11px] font-bold uppercase tracking-[0.18em] text-[#C8FF00]">
