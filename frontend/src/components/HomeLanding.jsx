...
function MarketOverviewGrid() {
  ...
  const loadMarkets = useCallback(async () => {
    try {
      ...
      for (const { symbol } of OVERVIEW_ASSETS) {
        const px = tapePriceFromMap(map, symbol, "USDT", "crypto");
        if (!(px > 0)) continue;
        next[symbol] = px;
        targets.current[symbol] = px;
        if (displays.current[symbol] == null) {
          displays.current[symbol] = px;
        }
      }
  ...
  useEffect(() => {
    loadMarkets();
    const poll = setInterval(loadMarkets, 8000);
...
export default function HomeLanding({ user, walletUsdt = 0, liveEarnings = 0, onStartTrading }) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-[#00C2B3]/20 px-5 py-8 shadow-[0_0_40px_rgba(0, 194, 179,0.08)] sm:px-8 sm:py-10 md:px-10 md:py-12">
        <HeroMediaSlider />
        ...
        <div className="text-center md:text-left">
          ...
          >
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#00C2B3] px-5 py-3 text-sm font-bold text-[#1a1400] shadow-[0_0_24px_rgba(0, 194, 179,0.4)] transition hover:bg-[#5EEAD4]"
            >
      ...
      {/* Candlestick desks: desktop/tablet only. Hidden on mobile after sign-in. */}
      <div className="hidden md:block">
        <LiveMarketDesks />
      </div>
